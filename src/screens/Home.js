...
import {
  ...
  ScrollView, TextInput, Modal, RefreshControl,
...
import {
  ...
  query, where, getDocs, updateDoc, doc, getDoc, arrayUnion
...
const reverseGeocode = async (lat, lng) => {
  try {
    ...
    return a.suburb || a.neighbourhood || a.quarter || a.town || a.village || a.city_district || a.city || null;
  } catch (e) {
...
export default function Home({ navigation }) {
  ...
  const getLocationDisplay = (item) => {
    ...
    const secondary = local ? `${city || ""}, ${state || ""}`.trim().replace(/^,|,$/, "") : state;
  ...
  const fetchNearbyGroups = async (locality) => {
    ...
    if (!uid) return;
    try {
      const nearbyQ = query(collection(db, "studySessions"), where("isPublic", "==", true));
      ...
      const nearby = snap.docs
        ...
        .filter(g => {
          ...
          if (g.members?.[uid] === true || g.members?.[uid] === false) return false;
          const groupLocality = (g.locationCity || "").trim().toLowerCase();
          return groupLocality && (groupLocality === localityLower || groupLocality.includes(localityLower) || localityLower.includes(groupLocality));
        });
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      const joinedActiveQ = query(collection(db, "studySessions"), where(`members.${uid}`, "==", true));
      const joinedInactiveQ = query(collection(db, "studySessions"), where(`members.${uid}`, "==", false));
      const [activeSnap, inactiveSnap] = await Promise.all([getDocs(joinedActiveQ), getDocs(joinedInactiveQ)]);
      const allJoined = [
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([fetchSessions(), userLocality ? fetchNearbyGroups(userLocality) : Promise.resolve()]);
    setRefreshing(false);
  ...
  const handleCreateSession = async () => {
    ...
    try {
      ...
      setIsPublic(false); setGroupLocationQuery(""); setGroupLocationSelected(null);
      setModalVisible(false);
      fetchSessions();
      // Navigate to StudyGroup with the new session
      const q = query(collection(db, "studySessions"), where("host", "==", uid), where("pin", "==", pin));
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (snapshot.empty) { Alert.alert("Not Found", "No session found with that PIN."); return; }
      ...
      if (sessionData.members?.[uid] === true) {
        setJoinModalVisible(false); setJoinPin("");
      ...
      if (sessionData.members?.[uid] === false) {
        ...
        setJoinModalVisible(false); setJoinPin(""); fetchSessions();
        navigation.navigate("StudyGroup", { sessionId });
        return;
      }
      const alreadyRequested = (sessionData.joinRequests || []).some(r => r.uid === uid);
      if (alreadyRequested) { Alert.alert("Already Requested", "Your join request is pending host approval."); return; }
  ...
  const handleApprove = async (req) => {
    try {
      ...
      const updated = (sessionDoc.data()?.joinRequests || []).filter(r => r.uid !== req.uid);
      await updateDoc(sessionRef, { [`members.${req.uid}`]: true, joinRequests: updated });
      fetchSessions();
  ...
  const handleDecline = async (req) => {
    try {
      ...
      fetchSessions();
    } catch (e) { console.log("Decline error:", e.message); }
  };
  ...
  const handleRequestNearby = async (group) => {
    const uid = auth.currentUser?.uid;
    const alreadyRequested = (group.joinRequests || []).some(r => r.uid === uid);
    if (alreadyRequested) { Alert.alert("Already Requested", "Your join request is pending host approval."); return; }
    try {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderRow}>
            ...
            {localityLoading && <ActivityIndicator size="small" color="#aaa" style={{ marginLeft: 6 }} />}
          </View>

          {!userLocality && !localityLoading && (
            <View style={styles.locationPrompt}>
              <Ionicons name="location-outline" size={20} color="#bbb" />
              <Text style={styles.locationPromptText}>Allow location access to see nearby groups</Text>
            </View>
          )}
          ...
          {userLocality && nearbyGroups.length === 0 && (
            <View style={styles.locationPrompt}>
              ...
              <Text style={styles.locationPromptText}>No public groups found near {userLocality}</Text>
            </View>
          )}
          ...
          {nearbyGroups.length > 0 && (
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {nearbyGroups.map(group => (
                <View key={group.id} style={[styles.sessionCard, styles.nearbyCard]}>
                  <View style={styles.sessionCardTop}>
                  ...
                  <Text style={styles.nearbyLocation} numberOfLines={1}>
                    <Ionicons name="location-outline" size={11} /> {group.locationDisplay || group.locationCity}
                  </Text>
                  <TouchableOpacity style={styles.requestBtn} onPress={() => handleRequestNearby(group)}>
        ...
        {/* Empty state */}
        {myGroups.length === 0 && joinedGroups.length === 0 && pendingRequests.length === 0 && nearbyGroups.length === 0 && !localityLoading && (
            ...
            >
              ...
              <TouchableOpacity style={styles.toggleRow} onPress={() => setIsPublic(!isPublic)}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.toggleSub}>
                    {isPublic ? "Visible to nearby users" : "Only joinable via PIN"}
                  </Text>
                </View>
              </TouchableOpacity>
              ...
              {isPublic && (
                <View style={{ zIndex: 99 }}>
                  ...
                  <View style={[styles.locationInputWrapper, groupLocationSelected && styles.locationInputSelected]}>
                    <Ionicons name="location-outline" size={15} color="#aaa" style={{ marginRight: 6 }} />
                    <TextInput
                      ...
                      autoCorrect={false}
                    />
                    {locationSearching && <ActivityIndicator size="small" color="#2e7d32" />}
                    {groupLocationSelected && (
                      <TouchableOpacity onPress={() => { setGroupLocationQuery(""); setGroupLocationSelected(null); }}>
                        <Ionicons name="close-circle" size={16} color="#ccc" />
                      </TouchableOpacity>
                    )}
                  </View>

                  {groupLocationSuggestions.length > 0 && !groupLocationSelected && (
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              ...
                              setGroupLocationSelected({ city: main, display });
                              setGroupLocationQuery(display);
                              setGroupLocationSuggestions([]);
                            }}
                          >
                            <Ionicons name="location-outline" size={13} color="#888" style={{ marginRight: 6 }} />
                            <View style={{ flex: 1 }}>
                              <Text style={styles.locationItemMain}>{main}</Text>
                              {secondary ? <Text style={styles.locationItemSub} numberOfLines={1}>{secondary}</Text> : null}
                            </View>
                          </TouchableOpacity>
                        );
                      })}
      ...
      <View style={styles.bottomButtons}>
        ...
        <TouchableOpacity style={styles.button} onPress={() => { setGroupName(""); setSubject(""); setDescription(""); setIsPublic(false); setGroupLocationQuery(""); setGroupLocationSelected(null); setModalVisible(true); }}>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "#fff", borderBottomWidth: 0.5, borderColor: "#eee" },
  logo: { fontSize: 20, fontWeight: "800", color: "#2e7d32" },

  searchContainer: { flexDirection: "row", alignItems: "center", marginHorizontal: 16, marginTop: 10, backgroundColor: "#fff", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: "#eee" },
  ...
  quoteCard: { marginHorizontal: 16, marginTop: 16, backgroundColor: "#fffde7", borderRadius: 14, padding: 16, borderLeftWidth: 4, borderLeftColor: "#f0a500" },
  quoteText: { fontSize: 14, color: "#555", lineHeight: 22, fontStyle: "italic", marginBottom: 8 },
  ...
  badgeCount: { backgroundColor: "#e53935", borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 },
  badgeCountText: { fontSize: 11, color: "#fff", fontWeight: "700" },

  requestCard: { backgroundColor: "#fff", borderRadius: 12, padding: 14, marginBottom: 8, flexDirection: "row", alignItems: "center", justifyContent: "space-between", shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  requestInfo: { flex: 1 },
  requestUsername: { fontSize: 14, fontWeight: "700", color: "#222" },
  requestSession: { fontSize: 12, color: "#888", marginTop: 2 },
  requestActions: { flexDirection: "row", gap: 8 },
  declineBtn: { width: 34, height: 34, borderRadius: 17, borderWidth: 1.5, borderColor: "#ffcdd2", alignItems: "center", justifyContent: "center" },
  approveBtn: { width: 34, height: 34, borderRadius: 17, backgroundColor: "#2e7d32", alignItems: "center", justifyContent: "center" },

  sessionCard: { width: 160, marginRight: 12, backgroundColor: "#fff", borderRadius: 14, padding: 14, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  joinedCard: { backgroundColor: "#f0f7ff" },
  hostedCard: { backgroundColor: "#f1f8e9" },
  nearbyCard: { backgroundColor: "#f3e5f5" },
  sessionCardTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
  hostBadge: { backgroundColor: "#e8f5e9", borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
  hostBadgeText: { fontSize: 9, fontWeight: "700", color: "#2e7d32", letterSpacing: 0.5 },
  memberBadge: { backgroundColor: "#e3f2fd" },
  memberBadgeText: { color: "#1565c0" },
  publicBadge: { flexDirection: "row", alignItems: "center", gap: 3, backgroundColor: "#ede7f6", borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
  publicBadgeText: { fontSize: 9, fontWeight: "700", color: "#6a1b9a", letterSpacing: 0.5 },
  sessionCardMembers: { fontSize: 12, color: "#aaa" },
  sessionCardName: { fontSize: 14, fontWeight: "700", color: "#222", marginBottom: 4 },
  sessionCardSubject: { fontSize: 12, color: "#888", marginBottom: 6 },
  nearbyLocation: { fontSize: 11, color: "#aaa", marginBottom: 10 },
  openBtn: { backgroundColor: "#2e7d32", borderRadius: 8, paddingVertical: 6, alignItems: "center" },
  rejoinBtn: { backgroundColor: "#1565c0" },
  openBtnText: { color: "#fff", fontSize: 12, fontWeight: "700" },
  requestBtn: { backgroundColor: "#6a1b9a", borderRadius: 8, paddingVertical: 6, alignItems: "center" },
  requestBtnText: { color: "#fff", fontSize: 11, fontWeight: "700" },

  locationPrompt: { flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 16 },
  locationPromptText: { fontSize: 13, color: "#bbb" },

  emptyState: { alignItems: "center", paddingTop: 60, paddingHorizontal: 40 },
  emptyTitle: { fontSize: 16, fontWeight: "700", color: "#bbb", marginTop: 16 },
  emptySubtitle: { fontSize: 13, color: "#ccc", textAlign: "center", marginTop: 6, lineHeight: 20 },

  bottomButtons: { flexDirection: "row", justifyContent: "space-around", padding: 12, borderTopWidth: 0.5, borderColor: "#eee", backgroundColor: "#fff" },
  button: { backgroundColor: "#2e7d32", paddingVertical: 12, paddingHorizontal: 32, borderRadius: 10, flexDirection: "row", alignItems: "center", gap: 6 },
  buttonText: { color: "#fff", fontWeight: "700", fontSize: 14 },

  overlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", alignItems: "center", padding: 20 },
  createOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", padding: 20 },
  createModalBox: { backgroundColor: "#fff", borderRadius: 16, padding: 20, maxHeight: "85%", flexGrow: 0 },
  modalScrollContent: { flexGrow: 1, justifyContent: "center", alignItems: "center" }, // used by join modal
  modalBox: { width: "100%", backgroundColor: "#fff", borderRadius: 16, padding: 20, maxHeight: "90%" },
  createModalBox: { width: "100%", backgroundColor: "#fff", borderRadius: 16, padding: 20, maxHeight: "90%" },
  modalTitle: { fontSize: 16, fontWeight: "700", color: "#222", marginBottom: 12 },
  input: { borderWidth: 1, borderColor: "#e0e0e0", borderRadius: 10, padding: 10, marginTop: 10, fontSize: 14 },
  joinNote: { fontSize: 12, color: "#aaa", marginTop: 8, marginLeft: 2 },
  modalButton: { backgroundColor: "#2e7d32", borderRadius: 10, padding: 13, marginTop: 14, alignItems: "center" },
  modalButtonText: { color: "#fff", fontWeight: "700", fontSize: 14 },
  grid: { flexDirection: "row", flexWrap: "wrap", marginTop: 10 },
  cube: { width: "30%", aspectRatio: 1, borderWidth: 1, borderColor: "#eee", borderRadius: 8, margin: 5, justifyContent: "center", alignItems: "center", padding: 4 },

  toggleRow: { flexDirection: "row", alignItems: "center", marginTop: 14, padding: 12, backgroundColor: "#f9f9f9", borderRadius: 10, flexWrap: "nowrap" },
  toggleSwitch: { width: 44, height: 24, borderRadius: 12, backgroundColor: "#ddd", justifyContent: "center", paddingHorizontal: 2 },
  ...
  locationLabel: { fontSize: 13, fontWeight: "600", color: "#555", marginTop: 12, marginBottom: 4 },
  locationInputWrapper: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: "#e0e0e0", borderRadius: 10, paddingHorizontal: 10, paddingVertical: 10, backgroundColor: "#fafafa" },
  locationInputSelected: { borderColor: "#2e7d32" },
  locationInput: { flex: 1, fontSize: 14, color: "#222" },
  locationDropdown: { backgroundColor: "#fff", borderWidth: 1, borderColor: "#e0e0e0", borderRadius: 10, marginTop: 4, shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 6, elevation: 6 },
  locationItem: { flexDirection: "row", alignItems: "flex-start", paddingHorizontal: 12, paddingVertical: 10, borderBottomWidth: 0.5, borderColor: "#f0f0f0" },
